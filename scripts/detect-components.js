 ...
 *        --head <sha-or-ref>
 *
 * Emits two GitHub-Actions outputs (JSON arrays):
...
const localArray = JSON.stringify([...changed]);
const namespacedArray =
  ns && changed.size ? JSON.stringify([...changed].map((k) => `${ns}-${k}`)) : '[]';

out('LOCAL_COMPONENTS', localArray);
out('NAMESPACED_COMPONENTS', namespacedArray);
